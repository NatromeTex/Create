...
`parity-progress.md` records the first gameplay changes adapted from upstream and their validation status. Both the unchanged-gameplay baseline and the modified 1.20.1 builds passed. The baseline had 58/60 GameTests pass; the modified build had 59/61 pass, including the new plough test. Both reported identical failures in `TestFluids.thresholdSwitch` and `TestItems.attributeFilters`, confirming that those failures predate these adaptations. The baseline checkout contains only mapping-mirror/report-path build adjustments. `validation.json` records the results. These results do not establish 26.1.2 compatibility.
